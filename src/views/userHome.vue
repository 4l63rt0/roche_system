      ...
      <v-layout row wrap class="justify-space-around">
        ...
        <v-flex xs12 sm6 md8>
          ...
          <v-flex row v-if="dailyScoreDateAvailable === true">
            <v-flex
              v-for="routine in dailyScore[0]['routine']"
              :key="routine.id"
              class="mr-1 ml-1"
            >
              <v-card class="mx-auto mb-4" max-width="275" hover>
                <v-img
...
export default {
  ...
  methods: {
    ...
    applyTask: function (
      ...
      this.dailyScore[0]["routine"][routineName]["kid"][kidName][
        "status"
      ] = !kidStatus;
  ...
  mounted: function () {
    this.time();
    let url =
      "https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/cf6831b0376f95a4b8578f907855d955/27.5455981,-99.8153993";
    axios
      .get(url)
      .then((resp) => {
